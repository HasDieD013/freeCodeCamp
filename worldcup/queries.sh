...
echo "$($PSQL "SELECT ROUND(AVG(winner_goals) + AVG(opponent_goals),16) AS media FROM games")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM games FULL JOIN teams ON games.winner_id = teams.team_id OR games.opponent_id = teams.team_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"
